...
@LastEditTime: 2020-03-21 12:17:03
'''
import re
import xlwt
import xlrd
...
if __name__ == "__main__":
    ...
    while line:
        ...
        if flag_keyword:
            ...
            subfileName = filter_list[keyword_index]
            keyword_index += 1

        if flag_keyword or check:
            if flag_keyword:
                print("Processing %s" % subfileName)
            re_result = re.search(r'(Power of board is :)(\d{2})', line)
            if re_result:
                Write_to_txt(subfileName)
